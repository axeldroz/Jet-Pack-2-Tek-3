...
** Last update Wed Jul 13 23:30:41 2016 Loïc GIGOMAS
*/
...
static t_descr	*init_descr(int port, char *host)
{
  ...
  if (!(des = malloc(sizeof(*des))) || !(memset(des, 0, sizeof(*des)))
      || (des->cli = new(t_tcpnetc, host, port)) == NULL
      || (des->map = new(t_game_map)) == NULL
      || (des->players = new(t_map, &comp_id)) == NULL
      || graph_init() == -1)
    return (NULL);
  if (graph_create_window(&des->win, (SDL_Rect){0, 0,
...
int		main(int ac, char **av)
{
  ...
  if ((opt = new(t_getopt)) == NULL
      ...
      || opt_register(opt, 'h', ePCHAR, MANDATORY) != OPT_NOERR)
    return (84);
